plot.4 <- function(file.name = "household_power_consumption.txt"){
    ...
    with(electric.data, points(Date_Time, Sub_metering_3, type = "l", 
                               col = "blue"))
    legend("topright", names(electric.data[, 7:9]), 
           col = c("black", "red", "blue"), lty = 1)
    with(electric.data, plot(Date_Time, Voltage, type = "l", 
                             main = "",  
                             xlab = "datetime", 
                             ylab = "Voltage"))
    with(electric.data, plot(Date_Time, Global_reactive_power, type = "l", 
                             main = "",  
                             xlab = "datetime", 
                             ylab = names(electric.data[, 4])))
